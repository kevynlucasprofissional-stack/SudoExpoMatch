...
} from "@/features/staff/useOperationalStats";
  ...
  return (
    <Card className="p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Kpi
          label="Apresentados"
          value={`${s.rate_presented.toFixed(1)}%`}
        />
        <Kpi
          label="Contato trocado"
          value={`${s.rate_contact_exchanged.toFixed(1)}%`}
        />
        <Kpi
          label="Concluídas"
          value={`${s.rate_completed.toFixed(1)}%`}
        />
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          {(Object.keys(CONNECTION_STATUS_LABEL) as ConnectionStatus[]).map(
            (st) => (
              ...
              >
                {CONNECTION_STATUS_LABEL[st]}:{" "}
                <strong>{s.by_status[st] ?? 0}</strong>
              </span>
            ),
          )}
        </div>
