...
ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_target_business_size_chk
  CHECK (target_business_size IS NULL OR target_business_size IN ('pequeno','medio','grande'));

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_target_business_type_chk
  CHECK (target_business_type IS NULL OR target_business_type IN ('comercio','industria','servico'));

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_target_segment_id_fkey
  FOREIGN KEY (target_segment_id) REFERENCES public.segments(id);

-- 2. Segmentos selecionáveis no perfil
ALTER TABLE public.segments
  ADD COLUMN IF NOT EXISTS profile_selectable boolean NOT NULL DEFAULT true;

UPDATE public.segments SET profile_selectable = false
 WHERE id IN ('comercio','industria','servicos');

-- 3. Catálogo devolve a flag (taxonomia intacta)
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'segments', (SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'profile_selectable', s.profile_selectable) ORDER BY s.sort_order), '[]'::jsonb)
...
BEGIN
  ...
  v_out := jsonb_build_object(
    ...
    'niche', v_row.niche,
    'target_business_size', v_row.target_business_size,
    'target_business_type', v_row.target_business_type,
    'target_segment_id', v_row.target_segment_id,
...
-- 5. Gravação do próprio perfil (adiciona target_*, valida server-side)
...
DECLARE
  ...
  v_t_size text := NULLIF(trim(COALESCE(_payload->>'target_business_size','')),'');
  v_t_type text := NULLIF(trim(COALESCE(_payload->>'target_business_type','')),'');
  v_t_segment text := NULLIF(trim(COALESCE(_payload->>'target_segment_id','')),'');
...
BEGIN
  ...
  END IF;
  IF v_t_size IS NOT NULL AND v_t_size NOT IN ('pequeno','medio','grande') THEN
    RAISE EXCEPTION 'invalid_target_business_size' USING ERRCODE='22023';
  END IF;
  IF v_t_type IS NOT NULL AND v_t_type NOT IN ('comercio','industria','servico') THEN
    RAISE EXCEPTION 'invalid_target_business_type' USING ERRCODE='22023';
  END IF;
  IF v_t_segment IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.segments WHERE id=v_t_segment) THEN
    RAISE EXCEPTION 'invalid_target_segment' USING ERRCODE='22023';
  END IF;
  ...
  IF v_id IS NULL THEN
    INSERT INTO public.profiles(owner_id, event_id, name, company, city, neighborhood,
      segment_id, summary, business_size, business_type, niche,
      target_business_size, target_business_type, target_segment_id, is_demo)
    VALUES (v_uid, v_event, v_name, v_company, v_city, v_neighborhood,
      v_segment, v_summary, v_business_size, v_business_type, v_niche,
      v_t_size, v_t_type, v_t_segment, false)
  ...
  ELSE
    UPDATE public.profiles
       SET name=v_name, company=v_company, city=v_city, neighborhood=v_neighborhood,
           ...
           target_business_size=v_t_size, target_business_type=v_t_type,
           target_segment_id=v_t_segment,
           updated_at=now()
     WHERE id=v_id;
  END IF;
...
-- 6. Admin: detalhe do participante inclui perfil procurado
