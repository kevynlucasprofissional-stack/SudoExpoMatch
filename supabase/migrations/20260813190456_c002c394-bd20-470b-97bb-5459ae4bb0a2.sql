ALTER TABLE public.match_reasons
  ADD COLUMN IF NOT EXISTS profile_need_id uuid REFERENCES public.profile_needs(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS profile_offer_id uuid REFERENCES public.profile_offers(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS taxonomy_relation_id uuid REFERENCES public.taxonomy_relations(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS rationale text,
  ADD COLUMN IF NOT EXISTS relation_weight int;

ALTER TABLE public.match_reasons
  DROP CONSTRAINT IF EXISTS match_reasons_relation_weight_check;
ALTER TABLE public.match_reasons
  ADD CONSTRAINT match_reasons_relation_weight_check
  CHECK (relation_weight IS NULL OR (relation_weight >= 1 AND relation_weight <= 100));

CREATE INDEX IF NOT EXISTS match_reasons_taxonomy_relation_id_idx ON public.match_reasons(taxonomy_relation_id);
CREATE INDEX IF NOT EXISTS match_reasons_profile_need_id_idx ON public.match_reasons(profile_need_id);
CREATE INDEX IF NOT EXISTS match_reasons_profile_offer_id_idx ON public.match_reasons(profile_offer_id);
...
'Matcher v2.3. Pesos literais preservados (55/25/10/5/3/2). Complementaridade direcional via taxonomy_relations (relation_type=complements, active): perspectiva "me" usa minha necessidade (from) -> oferta do outro (to); perspectiva do outro usa necessidade dele (from) -> minha oferta (to). Selecao deterministica da melhor relacao (weight DESC, r.id, need.id, offer.id; LIMIT 1) sem double count. Threshold: weight >= 40. Pontos = LEAST(30, round(weight*0.30)) => 12..30. O reason relacao_complementar registra profile_need_id, profile_offer_id, taxonomy_relation_id, relation_weight bruto e rationale (fallback neutro quando null), tanto no JSON quanto em match_reasons.';