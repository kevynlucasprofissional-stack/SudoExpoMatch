...
- **Correção desta branch**: `/admin/taxonomia` agora envia `selectedEventId` também ao `TaxonomyItemSheet`; antes a lista respeitava o evento selecionado, mas o detalhe/mutações ainda recebiam `EVENT_ID` fixo.

### Suporte ao participante

A branch incorporou o último `main`, incluindo os botões de suporte por WhatsApp em `ParticipantHeader.tsx` e `ProfileCard.tsx` e o teste correspondente em `multi-eventos-checkin.test.ts`.
