-- ==============================================================================
-- Migration: Ordenação de Matches — 'Agora não' no final da fila
-- Data: 2026-09-11
-- Descrição:
--   Atualiza a RPC public.list_own_matches_v2 para enviar matches marcados como
--   'agora_nao' pelo participante para o final da fila, mantendo no topo as
--   possibilidades em aberto ('sem_decisao') e matches ativos, ordenados por
--   alta sinergia mútua.
-- ==============================================================================
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(to_jsonb(e) ORDER BY
    -- 0) Matches marcados como 'agora_nao' vão para o final da fila
    CASE WHEN e.my_decision = 'agora_nao' THEN 1 ELSE 0 END ASC,
...
COMMENT ON FUNCTION public.list_own_matches_v2(text) IS
  'Retorna matches do participante logado com ordenação prioritária para duplas de alta sinergia mútua no topo e matches marcados como agora_nao no final da fila.';
