...
DECLARE
  ...
  v_comp_me int;      -- maior weight (1..100) de relacao complementar a favor de "me"
  v_comp_other int;   -- maior weight (1..100) de relacao complementar a favor do outro
  v_comp_pts_me int;
  v_comp_pts_other int;
  v_comp_min_weight CONSTANT int := 40;  -- threshold: abaixo disso a relacao e' ignorada
  v_comp_max_pts CONSTANT int := 30;     -- teto de pontos complementares por perspectiva
...
BEGIN
  ...
  -- Desativa apenas matches sem conexao associada (preserva historico).
  ...
  LOOP
    ...
    -- Perspectiva "me": minha necessidade (from) -> oferta do outro (to).
    -- Agregacao por MAX(weight) sobre relacoes DISTINTAS: uma mesma relacao nunca
    -- pontua duas vezes, mesmo casando com varios pares de itens.
    ...
    -- Perspectiva do outro: necessidade dele (from) -> minha oferta (to).
    ...
    -- Formula: pontos = LEAST(30, round(weight * 0.30)); weight 40 => 12 pts, 100 => 30 pts.
    ...
    -- Kinds literais preservados; 'complementar' apenas quando NAO ha sinal literal.
  ...
  -- Auditoria minima (sem PII): registra apenas contagem, versao e IDs internos.
...
'Matcher v2.3. Pesos literais preservados (55/25/10/5/3/2). Complementaridade direcional via taxonomy_relations (relation_type=complements, active): perspectiva "me" usa minha necessidade (from) -> oferta do outro (to); perspectiva do outro usa necessidade dele (from) -> minha oferta (to). Agregacao MAX(weight) sobre relacoes distintas (sem double count). Threshold: weight >= 40. Pontos = LEAST(30, round(weight*0.30)) => 12..30. Sinal complementar sozinho gera match kind=complementar; kinds literais (direto/inverso/bidirecional/hibrido) tem precedencia.';