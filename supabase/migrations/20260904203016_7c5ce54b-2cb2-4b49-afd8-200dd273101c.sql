CREATE OR REPLACE FUNCTION public._normalize_br_phone(_raw text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
REVOKE ALL ON FUNCTION public.lookup_profile_by_phone(text, text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.claim_profile_by_phone_simple(text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.lookup_profile_by_phone(text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.claim_profile_by_phone_simple(text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public._normalize_br_phone(text) TO authenticated, service_role;